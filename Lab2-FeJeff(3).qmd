...
dados|>
  filter(sex == "f")|>
  rename("Larg. Crânio" = skullw, "Comp. Total" = totlngth)|>
  summarytools::descr(
    ...
    kbl(
      ...
      col.names =
        c("Min", "Q1", "Med", "Média", "Q3", "Max", "D. Pad", "CV")
    )|>
...
d1+d1.1+plot_annotation(
  title = "Figura 8: Relação entre votos recebidos por Bush e por Bachanan nos condados \n da Flórida. (a) Dados completos, (b) Sem dados discrepantes",
...
Uma maior compreenção da dispersão dos dados das votações dos candidatos, optou-se por analizar a variabilidade dos dados de votação dos candidatos. Na Tabela 5 é possível verificar uma maior variabilidade dos votos do candidato Bush em todas as análise realizadas, o que se deve principalmente a quantidade muito superior de votos recebidos.  
...
## Amplitude total
amplitude1 <- max(dados.sem$BUSH)- min(dados.sem$BUSH)
amplitude2 <- max(dados.sem$BUCHANAN)- min(dados.sem$BUCHANAN)
## Desvio médio absoluto
desvio_medio_abs1<-round(sum(abs(dados.sem$BUSH-mean(dados.sem$BUSH))),0)
desvio_medio_abs2<-round(sum(abs(dados.sem$BUCHANAN-mean(dados.sem$BUCHANAN))),0)
## Variância
variancia1<-round(var(dados.sem$BUSH),0)
variancia2<-round(var(dados.sem$BUCHANAN),0)
## Desvio Padrão
desvio_padrão1 <- round(sd(dados.sem$BUSH),0)
desvio_padrão2 <- round(sd(dados.sem$BUCHANAN),0)
## Coeficiente de variação
coef_var1<-round((desvio_padrão1/mean(dados.sem$BUSH))*100,0)
coef_var2<-round((desvio_padrão2/mean(dados.sem$BUCHANAN))*100,0)

resumo1 <- cbind(amplitude1,desvio_medio_abs1,variancia1,desvio_padrão1, coef_var1)
resumo2 <- cbind(amplitude2,desvio_medio_abs2,variancia2,desvio_padrão2, coef_var2)
resumo <- rbind(resumo1, resumo2)
rownames(resumo) = c("Bush","Buchanan")
colnames(resumo) = c("Amplit.","Desvio abs","Variancia","D. Padrão",                          "C. Var.")
resumo |> kbl(
      caption = "Medidas de Variabilidade dos votos dos candidatos.",
      format.args=list(big.mark=".", decimal.mark=","),
      digits = 2, align = "c",
      row.names = T
    )|>
...
(d1+d1.1)/(d2+d2.2) + plot_annotation(
  title = "Figura 9: Relação entre votos recebidos por Gore e por Bachanan nos condados \n da Flórida. (a) Dados completos (Gore), (b) Sem dados discrepantes (Gore), \n (c) Dados completos (Total), (d) Sem dados discrepantes (Total)", 
  caption = "Fonte: 2000 OFFICIAL PRESIDENTIAL GENERAL ELECTION RESULTS - USA") &
  theme_bw(base_size = 8) &
  theme(
    legend.position = "none",
...
intercepto = round(reg$coefficients[1][[1]],0)
c.angular = round(reg$coefficients[2][[1]],2)
resposta = round((152846 - intercepto)/c.angular, 0)
```

A Figura 10 mostra a reta de regressão linear obtida, que tem como função $f(x) =$ `r intercepto` $+$ `r c.angular`$x$. Com base nesta regressão é possível inferir que caso o candidato Bush tivesse recebido 152.846 votos em um determinado condado, a votação do canditato Buchanan seria de aproximandamente `r resposta` votos.
...
d1+plot_annotation(
  title = "Figura 10: Relação entre votos recebidos por Bush e por Bachanan nos condados \n da Flórida e sua regressão linear.") &
  theme_bw(base_size = 8) &
  theme(
    legend.position = "none",
    plot.tag.position = c(0, 1),
...
Executando o código acima, obtém-se a estimação por mínimos quadrados de `r votos` votos. Observa-se que por regressão linear a resposta foi de `r resposta` votos, desta forma, percebe-se que ambos os métodos foram similares na predição da votação do candidato Buchanan.
